...
def _default_dir() -> Path:
    """Resolve the recorder data directory.

    Order:
      1. $MESHTASTIC_MCP_DATA_DIR/.mtlog (explicit override)
      2. platformdirs user-data dir (when available)
      3. <cwd>/.mtlog (fallback)

    Decoupled from the firmware tree: the standalone package is not anchored to a
    firmware checkout, so the old `parents[3]/.mtlog` (firmware-repo-relative) anchor
    is gone.
    """
    override = os.environ.get("MESHTASTIC_MCP_DATA_DIR")
    if override:
        return Path(override).expanduser().resolve() / ".mtlog"
    try:
        import platformdirs

        return Path(platformdirs.user_data_dir("meshtastic-mcp")) / ".mtlog"
    except Exception:
        return Path.cwd() / ".mtlog"
...
class Recorder:
    ...
    def _on_node_updated(self, node: dict[str, Any] | None = None, interface: Any = None) -> None:
