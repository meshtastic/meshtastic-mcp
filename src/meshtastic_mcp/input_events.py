...
def coerce_event_code(value: int | str | InputEventCode) -> int:
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            raise ValueError(f"unknown event code name {value!r}; known: {known}") from exc
    raise TypeError(f"event_code must be int|str|InputEventCode, got {type(value).__name__}")
