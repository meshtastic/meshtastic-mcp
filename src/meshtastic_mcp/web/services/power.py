...
class AmbiguousPort(RuntimeError):
    ...
    def __init__(self, candidates: list[dict]) -> None:
        slots = ", ".join(f"{c['location']}:{c['port']}" for c in candidates)
        super().__init__(
            f"{len(candidates)} hub ports share this device's VID ({slots}) — "
            "pin the right one in device settings ▸ USB power port"
        )
...
def candidates_for(device: dict) -> list[dict]:
    ...
    if vid is None:
        return []
    return [{"location": loc, "port": port} for loc, port in uhubctl.find_port_for_vid(vid)]
...
def power_slot(location: str, port: int, action: str) -> dict:
    """Drive a power action against an explicit hub slot (used by identify, which
    walks ports rather than resolving from a device)."""
    if action not in _ACTIONS:
        raise ValueError(f"unknown power action: {action}")
    return _ACTIONS[action](location, port)
