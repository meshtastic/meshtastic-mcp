...
def _normalize_rotation(rotation: int) -> int:
    ...
    return round((rotation % 360) / 90.0) * 90 % 360
...
async def for_device(db: Database, serial: str) -> dict | None:
    row = await db.fetchone(f"SELECT {_COLS} FROM cameras WHERE device_serial=? LIMIT 1", (serial,))
    return _to_dict(row)
...
async def set_mirror(db: Database, cid: int, mirror: bool) -> dict | None:
    ...
    await db.execute("UPDATE cameras SET mirror=? WHERE id=?", (int(bool(mirror)), cid))
    return await get(db, cid)
