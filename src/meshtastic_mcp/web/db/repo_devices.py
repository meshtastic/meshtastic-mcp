...
async def upsert_from_discovery(
    ...
    *,
    ...
    came_online = not existing["online"]  # was offline, now seen again
    await db.execute(
    ...
    # the card un-greys (port_changed would be False there).
    row["_came_online"] = came_online
    return row


async def upsert_native(db: Database, *, name: str, tcp_port: int, online: bool = True) -> dict:
...
async def set_friendly_name(db: Database, serial: str, name: str) -> dict | None:
    await db.execute("UPDATE devices SET friendly_name=? WHERE serial_number=?", (name, serial))
    return await get(db, serial)


async def set_env(db: Database, serial: str, env: str | None, *, locked: bool) -> dict | None:
...
async def update_enrichment(
    ...
    *,
    ...
    await db.execute(f"UPDATE devices SET {', '.join(sets)} WHERE serial_number=?", tuple(params))
    return await get(db, serial)
...
async def by_hub_slot(db: Database, *, location: str | None, port: int | None) -> dict | None:
    """The device pinned to a uhubctl ``(hub_location, hub_port)`` slot, or None.

    Lets the bake path resolve a bench role → its physical slot → the registry
    row, without depending on the collapsible VID ``role`` column (the only way
    to tell the three same-VID nRF52 boards apart). Newest-seen wins if more than
    one row ever shared a slot."""
    if not location or port is None:
        return None
    row = await db.fetchone(
        f"SELECT {_COLS} FROM devices WHERE hub_location=? AND hub_port=? "
...
async def mark_offline_except(db: Database, keep: set[str]) -> list[str]:
    ...
    for serial in newly:
        await db.execute("UPDATE devices SET online=0 WHERE serial_number=?", (serial,))
    return newly
...
async def online_with_env(db: Database) -> list[dict]:
    ...
    rows = await db.fetchall(
        f"SELECT {_COLS} FROM devices WHERE online=1 AND kind='usb' AND env IS NOT NULL"
    )
    return [_to_dict(r) for r in rows]
