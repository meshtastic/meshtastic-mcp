...
_COLS = "id, env, fw_sha, fw_branch, status, duration_s, artifact_dir, error, created_at"
...
async def create(db: Database, *, env: str, fw_sha: str, fw_branch: str | None, status: str) -> int:
...
async def set_status(
    ...
    *,
    ...
    await db.execute(
        "UPDATE builds SET status=?, duration_s=?, artifact_dir=?, error=? WHERE id=?",
...
async def get(db: Database, env: str, fw_sha: str) -> dict | None:
    ...
    row = await db.fetchone(
        f"SELECT {_COLS} FROM builds WHERE env=? AND fw_sha=? ORDER BY id DESC LIMIT 1",
        (env, fw_sha),
    )
    return _to_dict(row)
...
async def list_all(db: Database, limit: int = 100) -> list[dict]:
    rows = await db.fetchall(f"SELECT {_COLS} FROM builds ORDER BY id DESC LIMIT ?", (limit,))
