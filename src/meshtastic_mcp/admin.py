...
MODULE_CONFIG_SECTIONS = {f.name for f in localonly_pb2.LocalModuleConfig.DESCRIPTOR.fields}
...
def _coerce_enum(field: pb_descriptor.FieldDescriptor, value: Any) -> int:
    ...
    if isinstance(value, int):
        if enum_type.values_by_number.get(value) is None:
            raise AdminError(f"{field.name}: {value} is not a valid {enum_type.name} value")
        return value
    if isinstance(value, str):
        ...
        if ev is None:
            ...
            raise AdminError(
                f"{field.name}: {value!r} is not a valid {enum_type.name}. Valid: {valid}"
            )
        return ev.number
    raise AdminError(f"{field.name}: expected enum {enum_type.name}, got {type(value).__name__}")
...
def _coerce_scalar(field: pb_descriptor.FieldDescriptor, value: Any) -> Any:
    ...
    raise AdminError(f"{field.name}: unsupported field type {t}. Use raw protobuf for this field.")
...
def factory_reset(
    ...
    full=False → config returns to defaults; BLE bonds and the X25519 identity
    key are PRESERVED. full=True → full device wipe: BLE bonds cleared and a new
    identity keypair generated (peers must re-exchange NodeInfo before DMs
    decrypt again).

    The firmware dispatches on WHICH field is set, not its value, so we must set
    the correct field — `factory_reset_device` (full) vs `factory_reset_config`
    (config-only) — each with int 1. Building the AdminMessage directly (rather
    than `Node.factoryReset`) keeps us independent of the SDK, including the
    2.7.8 bool→int32 assignment bug.
    """
