...
# seed fixtures + generated v25 protos) lives in the Meshtastic firmware tree, so
# push_fake_nodedb is a firmware-capability tool. Resolve those paths from the firmware
# root; callers without a firmware checkout get a clear ConfigError (and the MCP tool is
# gated off entirely — see server._FIRMWARE_TOOLS).
def _firmware_root() -> pathlib.Path:
    return config.firmware_root()


def _seed_dir() -> pathlib.Path:
    return _firmware_root() / "test" / "fixtures" / "nodedb"


def _compile_script() -> pathlib.Path:
    return _firmware_root() / "bin" / "seed-json-to-proto.py"
...
def _compile_proto(jsonl_path: pathlib.Path, out_path: pathlib.Path) -> None:
    compile_script = _compile_script()
    if not compile_script.is_file():
        raise FixtureError(f"compile script missing at {compile_script}")
    cmd = [
        sys.executable,
        str(compile_script),
        "--in",
...
def _resolve_seed_jsonl(size: int, custom: str | None) -> pathlib.Path:
    ...
    if not p.is_file():
        raise FixtureError(
            f"missing committed seed at {p}. Run `./bin/regen-fake-nodedbs.sh` to generate it."
        )
    return p
        ...
        with connect(port=port) as iface:
            ...
            for offset in range(0, total_bytes, _XMODEM_CHUNK):
                ...
                while True:
                    ...
                    raise FixtureError(f"unexpected XModem control={ack.control} on seq={seq}")
