...
def _parse_time(value: Any, *, now: float | None = None) -> float:
    ...
    if not isinstance(value, str):
        raise ValueError(
            f"invalid time: {value!r}. Expected: epoch seconds (int/float), "
            "relative string ('-15m', '-2h', '-3d', 'now'), "
            "or ISO 8601 datetime ('2026-05-07T14:30:00')."
        )
    s = value.strip().lower()
    if s in ("", "now"):
    ...
    *,
    ...
    if grep:
        try:
            ...
            preview = (
                grep if len(grep) <= _REGEX_PREVIEW_MAX else f"{grep[:_REGEX_PREVIEW_TRUNCATE]}..."
            )
