...
def _mcp_server_root() -> pathlib.Path:
    ...
    for parent in (here.parent, *here.parents):
        if (parent / "pyproject.toml").is_file() and (parent / "run-tests.sh").is_file():
            return parent
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            f"error: textual is not installed ({exc}). Install with: pip install -e '.[test]'",
            file=sys.stderr,
        )
        return 2
    ...
    *,
            ...
            try:
                for line in iter(self._stream.readline, ""):
                    if self._stop.is_set():
                        break
                    self._app.post_message(PytestLine(source=self._source, line=line.rstrip("\n")))
            except Exception:
        ...
        def _poll_once(self) -> None:
            try:
                ...
                self._app.post_message(
                    PytestLine(source="stderr", line=f"[tui] device import failed: {exc!r}")
                )
                return
            rows: list[DeviceRow] = []
            try:
                ...
                self._app.post_message(
                    PytestLine(source="stderr", line=f"[tui] list_devices failed: {exc!r}")
                )
                return
            for d in raw:
    ...
    class FailureDetailScreen(tx.ModalScreen):
        ...
        def compose(self) -> Any:
            ...
            log = tx.RichLog(highlight=False, markup=False, wrap=False, id="failure-detail-log")
            yield log
            yield tx.Static(
    ...
    class CoverageModal(tx.ModalScreen):
        ...
        def compose(self) -> Any:
            ...
            yield tx.RichLog(highlight=False, markup=True, wrap=False, id="coverage-log")
            yield tx.Static(
        ...
        def on_mount(self) -> None:
            ...
            if not calls:
                ...
                return
            exercised = sorted(((n, c) for n, c in calls.items() if c > 0), key=lambda x: -x[1])
    ...
    class ReproducerResultModal(tx.ModalScreen):
        ...
        def __init__(self, archive_path: pathlib.Path, error: str | None = None) -> None:
    ...
    class TestTuiApp(tx.App):
        ...
        def __init__(self) -> None:
            ...
            # Cross-run history.
            self._history_store = _history_mod.HistoryStore(root / _HISTORY_RELATIVE, keep_last=40)
                ...
                with tx.Vertical(id="right-pane"):
                    ...
                    if self._ui_camera_enabled:
                        ...
                        with tx.Horizontal(id="uicap-pane"):
                            yield tx.Static("(waiting…)", id="uicap-image", markup=False)
                            yield tx.RichLog(
                                id="uicap-log",
                                highlight=False,
                                markup=False,
        ...
        def _header_text(self) -> str:
            ...
            spark = _history_mod.sparkline((r.duration_s for r in self._history_cache), width=20)
        ...
        def _spawn_pytest(self, extra_args: list[str]) -> None:
            ...
            threading.Thread(target=self._watch_exit, daemon=True, name="pytest-watch").start()
        ...
        def on_report_log_event(self, message: Any) -> None:
            ...
            if rt == "TestReport":
                ...
                if when == "setup" and outcome == "passed":
                    ...
                    self._refresh_device_status()
                elif when == "call" or (when == "setup" and outcome in ("failed", "skipped")):
                ...
                if (when == "call" and outcome in ("passed", "failed", "skipped")) or (
        ...
        def on_firmware_log_line(self, message: Any) -> None:
            ...
            if port and port not in self._fwlog_ports:
                ...
                try:
                    self.query_one("#fwlog-header", tx.Static).update(self._fwlog_header_text())
                except Exception:
                    pass
        ...
        def _refresh_device_status(self) -> None:
            ...
            for row in self._state.devices:
                role = row.role or "?"
                try:
                    dev_table.update_cell(row.port, "Status", self._status_for_role(role))
                except Exception:
        ...
        def _refresh_tier_row(self, tier: str) -> None:
            ...
            try:
                ...
                table.update_cell(tier, column_key="Skipped", value=str(counters.skipped))
                table.update_cell(tier, column_key="Running", value=str(counters.running))
                table.update_cell(tier, column_key="Remaining", value=str(counters.remaining))
                table.update_cell(tier, column_key="Progress", value=self._progress_bar(counters))
            except Exception:
        ...
        def _add_to_tree(self, leaf: LeafReport) -> None:
            ...
            file_node.add_leaf(f"{glyph} {_testname_of_nodeid(leaf.nodeid)}", data=leaf.nodeid)
        ...
        def _refresh_tree_leaf(self, leaf: LeafReport) -> None:
            ...
            for tier_node in tree.root.children:
                ...
                for file_node in tier_node.children:
                    if str(file_node.label).split(" ")[0] != _file_of_nodeid(leaf.nodeid):
                        continue
        ...
        def action_open_html_report(self) -> None:
            ...
            try:
                ...
                subprocess.Popen([opener, str(self._report_html)])
            except Exception:
                self.bell()
        ...
        def action_export_reproducer(self) -> None:
            ...
            try:
                ...
                self.push_screen(ReproducerResultModal(pathlib.Path("(none)"), error=repr(exc)))
                return
        ...
        def _reset_for_rerun(self) -> None:
            ...
            for tier_key in TIERS:
                tier_table.update_cell(tier_key, column_key="Passed", value="0")
                tier_table.update_cell(tier_key, column_key="Failed", value="0")
                tier_table.update_cell(tier_key, column_key="Skipped", value="0")
                tier_table.update_cell(tier_key, column_key="Running", value="0")
                tier_table.update_cell(tier_key, column_key="Remaining", value="0")
                tier_table.update_cell(tier_key, column_key="Progress", value="")
            tree = self.query_one("#test-tree", tx.Tree)
