...
<template>
  ...
  >
    ...
    <!-- Datadog forwarding status (global; configured in the Fleet tab panel) -->
    <span
      v-if="dd.status?.config.has_key"
      class="flex items-center gap-1.5 text-xs mono px-2 py-1 rounded-md bg-slate-900/70 border border-slate-800"
      :class="dd.status.stats.running ? 'text-emerald-400' : 'text-slate-500'"
      :title="
        dd.status.stats.running
          ? `Datadog: shipping logs (${dd.status.stats.sent_logs} sent)`
          : 'Datadog: configured but idle'
      "
    >
      <span
        class="w-1.5 h-1.5 rounded-full"
