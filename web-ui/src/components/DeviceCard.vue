...
// "Provisioned by the suite" = the bake (or a manual flash) stamped flashed_at.
// Without it an online-but-never-baked board looks identical to a baked one, so
// flag it: real USB boards only (native nodes are never flashed).
const isNative = computed(() => props.device.role === "native");
const provisioned = computed(() => props.device.flashed_at != null);
const neverBaked = computed(
  () => !isNative.value && props.device.online && !provisioned.value,
);
  ...
  >
    ...
    <div class="flex items-start gap-2.5">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2">
          ...
          <span
            v-if="neverBaked"
            class="text-[10px] px-1.5 py-0.5 rounded bg-rose-950/50 text-rose-400"
            title="online but never provisioned by FleetSuite — its firmware is unverified; run a bake to provision it"
            >unbaked</span
          >
        </div>
    ...
    <div class="text-[11px] flex items-center gap-1.5">
      ...
      <template v-if="provisioned">
        <span v-if="device.flashed_fw_sha" class="mono text-emerald-300/80">{{
          device.flashed_fw_sha.slice(0, 7)
        }}</span>
        <span v-else class="text-emerald-300/80">✓ baked</span>
        <span
          v-if="flashedDrift"
      ...
      <span
        v-else-if="neverBaked"
        class="text-rose-400/80"
        title="online but never provisioned by FleetSuite — run a bake to provision it"
        >never baked via FleetSuite</span
      >
