...
// Collapsed by default to save vertical space; persisted across reloads.
const collapsed = ref(localStorage.getItem("fs.cameras.collapsed") !== "false");
watch(collapsed, (v) => {
  localStorage.setItem("fs.cameras.collapsed", String(v));
  // Defer the (camera-activating) scan until the panel is first opened.
  if (!v && !scanned.value) scan();
});
...
// Scan on first open if the panel starts expanded (persisted state).
if (!collapsed.value) scan();
...
<template>
  <div
    class="card-rail rounded-xl border border-slate-700/80 bg-slate-900/60 p-4"
  >
    <div class="flex items-center gap-2" :class="{ 'mb-3': !collapsed }">
      <button
      ...
      >
        ...
        <span v-if="cameras.list.length" class="text-[11px] text-slate-500 mono"
          >{{ cameras.list.length }}</span
        >
      </button>
      <button
        v-if="!collapsed"
