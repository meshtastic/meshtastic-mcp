...
const identify = () => {
  if (
    !confirm(
      "Identify will briefly cut power to candidate hub ports one at a time " +
        "(those nodes reboot) and watch which one drops, then pin it. Continue?",
    )
  )
    return;
  ...
  act("identify", async () => {
    try {
      ...
      if (res.identified && res.slot) {
        ...
        throw new Error("couldn't detect a drop — assign manually");
      }
    } finally {
    ...
    <div v-if="!isNative">
      ...
      <div v-else class="flex gap-1.5 mt-1">
        ...
        <button
          @click="identify"
          :disabled="busy"
          class="px-2 py-1 rounded border border-indigo-700 text-indigo-300 hover:bg-indigo-600/20 disabled:opacity-40"
          title="power-cycle candidate ports and detect which one drops this node"
        >
      ...
      <p v-if="identifying" class="text-[10px] text-amber-400/80 mt-1">
        cutting power to candidate ports one at a time — nodes will blink offline;
        this can take up to ~10s per port
      </p>
    </div>
