...
const collapsed = ref(localStorage.getItem("fs.native.collapsed") !== "false");
watch(collapsed, (v) => localStorage.setItem("fs.native.collapsed", String(v)));
...
<template>
  <div class="card-rail rounded-xl border border-slate-700/80 bg-slate-900/60 p-4">
    <div class="flex items-center gap-2" :class="{ 'mb-3': !collapsed }">
      ...
      >
        <svg
          class="w-3 h-3 text-slate-500 transition-transform shrink-0"
        ...
        <span class="w-1 h-3.5 rounded-full bg-indigo-500/80 shrink-0" />
        <h3 class="section-label shrink-0">Native Nodes (Docker)</h3>
        <span v-if="info.nodes.length" class="text-[11px] text-slate-500 mono"
          >{{ info.nodes.length }}</span
        >
        <span
          v-if="!info.docker"
        ...
        >
        <span class="text-[11px] text-slate-600 mono truncate">{{
          info.image
        }}</span>
      </button>
    </div>
