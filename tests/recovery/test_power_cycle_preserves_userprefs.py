...
def test_lora_config_survives_power_cycle(
    ...
    assert probe.get("my_node_num") is not None, f"device {role!r} didn't respond after power-cycle"
    ...
    assert post_region == pre_region, (
        f"lora.region wiped by power-cycle on {role}: pre={pre_region!r} post={post_region!r}"
    )
    assert post_preset == pre_preset, (
        f"lora.modem_preset wiped by power-cycle on {role}: pre={pre_preset!r} post={post_preset!r}"
    )
