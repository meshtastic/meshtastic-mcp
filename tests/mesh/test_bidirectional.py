...
def test_bidirectional_mesh_communication(
    ...
    try:
        for role in roles:
            rx = ReceiveCollector(baked_mesh[role]["port"], topic="meshtastic.receive.text")
            rx.__enter__()
            collectors[role] = rx
        ...
        for sender, tag in tags.items():
            for receiver in roles:
                ...
                if got is None:
                    observed = [
                        p.get("decoded", {}).get("text") for p in collectors[receiver].snapshot()
                    ]
                    missing.append(
                        f"{sender}->{receiver}: tag {tag!r} not seen; receiver got {observed!r}"
                    )
