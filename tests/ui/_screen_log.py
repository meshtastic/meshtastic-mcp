    ...
    while time.monotonic() < deadline:
        for line in lines[start_idx:]:
            ...
            if evt.reason == reason:
                return evt
        time.sleep(poll_interval_s)
    raise TimeoutError(f"no frame with reason={reason!r} in {timeout_s}s; last={last!r}")
...
def assert_no_frame_change(
    ...
    *,
    ...
    start_idx = len(lines)
    time.sleep(wait_s)
    new = [e for e in (FrameEvent.parse(ln) for ln in lines[start_idx:]) if e is not None]
    if new:
