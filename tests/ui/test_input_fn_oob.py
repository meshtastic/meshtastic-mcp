...
def test_fn_f5_out_of_bounds(
    ...
    if start.count > 5:
        pytest.skip(f"device has {start.count} frames; FN_F5 is in-bounds — not testing OOB here")
    ...
    assert post is not None, "UI wedged after OOB FN_F5 — RIGHT no longer produces frame log"
