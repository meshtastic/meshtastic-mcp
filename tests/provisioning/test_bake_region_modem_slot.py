...
def test_bake_sets_region_preset_and_slot(
    ...
    for role, state in baked_mesh.items():
        ...
        expected_preset = test_profile["USERPREFS_LORACONFIG_MODEM_PRESET"].rsplit("_", 2)[-2:]
        expected_preset_str = "_".join(expected_preset)

        assert live["region"] == expected_region, (
        ...
        assert (
            int(lora.get("channel_num", 0)) == test_profile["USERPREFS_LORACONFIG_CHANNEL_NUM"]
