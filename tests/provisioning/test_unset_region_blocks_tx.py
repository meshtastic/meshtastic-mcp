...
def test_unset_region_blocks_tx(
    ...
    try:
        ...
        assert live.get("region") in (None, "UNSET", "UNSET_0", ""), (
            f"expected region UNSET after baking without region pref; got {live.get('region')!r}"
        )
        ...
        log_says_unset = any(
            marker in log.lower() for marker in ("region unset", "region is unset", "no region set")
        )
