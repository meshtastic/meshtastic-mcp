...
def recorder(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Recorder:
    ...
    monkeypatch.setattr("meshtastic_mcp.recorder.recorder._INSTANCE", None, raising=False)
...
class TestParseLogLine:
    ...
    def test_thread_freed_event(self) -> None:
        out = parse_log_line("++++++ Thread Router freed heap 1000 -> 1500 (500) ++++++")
        ev = out["heap_event"]
...
class TestRecorderDebugHeapSynthesis:
    ...
    def test_thread_leak_event_persists_on_log_row(self, recorder: Recorder) -> None:
        ...
        rows = [
            json.loads(r) for r in (recorder.base_dir / "logs.jsonl").read_text().splitlines() if r
        ]
...
class TestSerialTap:
    def test_serial_line_records_log_and_synthesizes_heap(self, recorder: Recorder) -> None:
    ...
    def test_serial_line_handler_swallows_exceptions(self, recorder: Recorder) -> None:
...
class TestExtractTelemetry:
    def test_local_stats_camel(self) -> None:
        pkt = {
            "decoded": {
                "telemetry": {"localStats": {"heap_total_bytes": 1000, "heap_free_bytes": 600}}
            }
        }
    ...
    def test_device_metrics_snake(self) -> None:
        pkt = {"decoded": {"telemetry": {"device_metrics": {"battery_level": 88, "voltage": 4.1}}}}
        out = extract_telemetry(pkt)
        assert out is not None
