...
def test_build_then_cache_hit(tmp_path, monkeypatch):
    ...
    async def go():
        ...
        await orch.enqueue(["heltec-v3"], sha="def456", branch="develop")
        await asyncio.gather(*orch._inflight.values())
        assert calls["n"] == 2
...
def test_build_logs_use_separate_topic(tmp_path, monkeypatch):
    """Regression: build log lines must NOT ride the build.update topic. The
    store replaces the whole row on build.update, so a partial {id, log} frame
    would wipe the row's status and hide build progress. Logs go on build.log."""
    monkeypatch.setenv("MESHTASTIC_MCP_ARTIFACT_DIR", str(tmp_path / "artifacts"))

    def fake_build(env: str, out: Path, log_cb) -> bool:
        log_cb("Compiling foo.cpp")
        log_cb("Linking firmware.elf")
        out.mkdir(parents=True, exist_ok=True)
        (out / f"firmware-{env}.factory.bin").write_bytes(b"\x00")
        return True
    ...
    async def go():
        ...
        hub.bind_loop(asyncio.get_running_loop())
        orch = builder.BuildOrchestrator(db, hub, build_fn=fake_build)
        await orch.enqueue(["heltec-v3"], sha="abc123", branch="develop")
        await asyncio.gather(*orch._inflight.values())
        await asyncio.sleep(0.05)  # drain threadsafe-scheduled log publishes
        await db.close()

        updates = [d for t, d in hub.frames if t == "build.update"]
        logs = [d for t, d in hub.frames if t == "build.log"]

        # Every build.update frame is a real row carrying a status — a log line
        # never masquerades as one.
        assert updates and all("status" in d for d in updates)
        assert {d["status"] for d in updates} >= {"queued", "building", "success"}
        # Logs are their own stream: carry id + line, never a status.
        assert logs and all(d.get("line") and "status" not in d for d in logs)
        assert any(d["line"] == "Compiling foo.cpp" for d in logs)
