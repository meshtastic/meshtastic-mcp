...
class TestNormalizeTcpEndpoint:
    ...
    def test_host_port(self) -> None:
        assert connection.normalize_tcp_endpoint("localhost:5000") == "tcp://localhost:5000"

    def test_full_url(self) -> None:
        assert connection.normalize_tcp_endpoint("tcp://1.2.3.4") == "tcp://1.2.3.4:4403"
        assert connection.normalize_tcp_endpoint("tcp://1.2.3.4:9999") == "tcp://1.2.3.4:9999"
...
class TestResolvePort:
    ...
    def test_no_port_no_devices_errors(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch.object(devices, "list_devices", return_value=[]):
            with pytest.raises(connection.ConnectionError, match="No Meshtastic devices"):
                connection.resolve_port(None)

    def test_no_port_one_candidate_selected(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("MESHTASTIC_MCP_TCP_HOST", raising=False)
    ...
    def test_no_port_multiple_candidates_errors(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("MESHTASTIC_MCP_TCP_HOST", raising=False)
        fake = [
    ...
    def test_env_var_surfaces_tcp_via_devices(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestDevicesTcpEntry:
    ...
    def test_invalid_env_var_does_not_pre_empt_real_usb_devices(
        ...
        with (
            patch("meshtastic_mcp.devices.list_ports.comports", return_value=[fake_port]),
            patch(
    ...
    def test_likely_tcp_entry_wins_tiebreak_over_usb(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestConnectRoutesTcp:
    def test_connect_uses_tcp_interface_for_tcp_port(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with (
            patch("meshtastic.tcp_interface.TCPInterface") as mock_tcp,
            patch("meshtastic.serial_interface.SerialInterface") as mock_serial,
        ):
    ...
    def test_connect_releases_lock_on_tcp_failure(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("MESHTASTIC_MCP_TCP_HOST", raising=False)
