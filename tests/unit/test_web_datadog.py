...
def test_scrub_modes():
    ...
    assert Scrubber("coarse").scrub("lat=37.774929 lon=-122.419416") == "lat=37.8 lon=-122.4"
...
def test_log_debug_skipped_but_panics_always_ship():
    base = {"host": "h", "base_tags": [], "port_tags": {}, "scrubber": Scrubber("off")}
    assert dd.log_to_dd({"level": "DEBUG", "line": "x"}, ship_debug=False, **base) is None
    assert dd.log_to_dd({"level": "DEBUG", "line": "x"}, ship_debug=True, **base) is not None
    # an un-leveled line (panic/backtrace) always ships
    assert (
        dd.log_to_dd({"level": None, "line": "Guru Meditation"}, ship_debug=False, **base)
        is not None
    )
...
def test_cursor_reader_partial_line_and_truncation(tmp_path):
    ...
    rows2, _cand2 = dd._read_live(p, cand, 100)
    assert rows2 == []
    p.write_text(
        json.dumps({"a": 1}) + "\n" + json.dumps({"a": 2}) + "\n" + json.dumps({"a": 3}) + "\n"
    )
...
def test_browser_intake_origin():
    assert (
        dd._browser_intake_origin("us5.datadoghq.com") == "https://browser-intake-us5-datadoghq.com"
    )
    assert dd._browser_intake_origin("datadoghq.eu") == "https://browser-intake-datadoghq.eu"
...
def test_config_masks_key_and_persists(tmp_path):
    async def go():
        ...
        await db.connect()
        try:
            cfg = dd.DDConfig(enabled=True, api_key="abcd1234efgh5678", site="us5.datadoghq.com")
            masked = cfg.masked()
