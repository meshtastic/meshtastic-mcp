...
def test_psk_seed_isolates_runs() -> None:
    ...
    assert lab_a_morning["USERPREFS_CHANNEL_0_PSK"] != lab_a_evening["USERPREFS_CHANNEL_0_PSK"]
    # Different labs, same time-of-day → different PSKs
    assert lab_a_morning["USERPREFS_CHANNEL_0_PSK"] != lab_b_morning["USERPREFS_CHANNEL_0_PSK"]

    # Re-deriving with the same seed yields the same PSK (reproducibility)
    lab_a_morning_again = userprefs.build_testing_profile(psk_seed="lab-a-2026-04-16-morning")
    assert (
        lab_a_morning["USERPREFS_CHANNEL_0_PSK"] == lab_a_morning_again["USERPREFS_CHANNEL_0_PSK"]
    )
