...
- **Reading a silent console as a dead device.** `set_debug_log_api(True)` makes the firmware stream its log as `LogRecord` protobufs over the StreamAPI *instead of* text on the console, so `serial_read` correctly returns nothing while the node is perfectly healthy — only `logs_window` sees anything. It is the same shape as a crashed board, and it has cost hours. `serial_read` now says so in a `hint` when a session has produced no output at all; believe it before concluding the hardware is broken.
- **Opening a monitor after the interesting part.** Boot is where firmware failures announce themselves, and by the time you have called `serial_open` the device booted minutes ago. Rebooting over the API needs the very port the monitor holds, so the two cannot be sequenced. Use `serial_open(reset=True)`, which pulses the auto-reset lines (RTS→EN, DTR→GPIO0) immediately before the monitor starts; check `reset_pulsed` in the response.
